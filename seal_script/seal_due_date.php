...
$sqlselect = "SELECT * FROM `$sealSTAT` WHERE `DueDate` = '" . $fivedays . "' and `receiveAccount` IS NOT NULL and `returnAccount` IS NULL AND `checkinAccount` IS NULL ORDER BY `index` ASC";
...
while ($row = mysqli_fetch_assoc($retval)) {
  ...
  $messagereq = "ILL# $illnum has a due date approaching: $duedate <br><br>
      ...
      This is an automated message from the SEAL ILL System. Responses to this email will be sent back to staff at Southeastern New York Library Resources Council. If you would like to conta
ct the other
library in this ILL transaction, email.";
  ...
  mail($email_to, $subject, $messagereq, $headers, "-f donotreply@senylrc.org");
}
