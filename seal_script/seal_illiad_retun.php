...
while ($row = mysqli_fetch_assoc($retval)) {
    ...
    if (strpos($status, 'Request Finished') !== false) {
        ...
        $sqlupdate2 = "\n UPDATE `$sealSTAT` SET `checkinAccount`='ILLiad',`returnAccount` = 'ILLiad', `IlliadStatus` = '$status' WHERE `index` = $sqlidnumb\n";
        echo $sqlupdate2;
        ...
        //if error happen let tech support know
            ...
            # for testing mail($to, $subject, $message, $headers, "-f donotreply@senylrc.org");
