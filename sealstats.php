...
if ( ($_SERVER['REQUEST_METHOD'] == 'POST')   || ( isset($_GET{'page'}))   ) {
  if ( $_REQUEST['stattype'] == 'wholesystem')   {
	...
	$GETREQUESTCOUNTSQLL= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem` LIKE '%$libsystem%'  and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00' ";
	$retval = mysqli_query($db, $GETREQUESTCOUNTSQLL);
	$row_cnt = mysqli_num_rows($retval);
	#Get total filled requests
	$FINDFILL= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem` LIKE '%$libsystem%' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  and  Fill =1 ";
	$retfilled =   mysqli_query($db, $FINDFILL);
	$row_fill = mysqli_num_rows($retfilled);
	#Get percentage fill
	$percentfill = $row_fill/$row_cnt;
	...
	#Get total not filled requests
	$FINDNOTFILL= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem` LIKE '%$libsystem%' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  and  Fill =0 ";
	$retnotfilled =   mysqli_query($db, $FINDNOTFILL);
	$row_notfill = mysqli_num_rows($retnotfilled);
	...
	#Get total requests expired
	$FINDEXPIRE= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem` LIKE '%$libsystem%' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  and  Fill =4 ";
	$retexpire =   mysqli_query($db, $FINDEXPIRE);
	$row_expire = mysqli_num_rows($retexpire);
	#Get percentage fill
	$percentexpire = $row_expire/$row_cnt;
	...
	#Get total requests not answered
	$FINDNOANSW= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem` LIKE '%$libsystem%' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  and  Fill =3 ";
	$retnoansw =   mysqli_query($db, $FINDNOANSW);
	$row_noansw = mysqli_num_rows($retnoansw);
	#Get percentage fill
	$percentnoansw = $row_noansw/$row_cnt;
	...
	#Get total requests canceled
	$CANANSW= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem` LIKE '%$libsystem%' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  and  Fill =6 ";
	$canansw =   mysqli_query($db, $CANANSW);
	$row_cancel = mysqli_num_rows($canansw);
	#Get percentage fill
	$percentcancel = $row_cancel/$row_cnt;
    ...
    if (strlen($libsystem)>1){
      ...
      #Find which systems they sent request to
      $destsystem=" SELECT distinct (`ReqSystem` )  FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem`='$libsystem' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
	 
      $destsystemq = mysqli_query($db, $destsystem);
      #loop through the results of destion systems
      while ($row = mysqli_fetch_assoc($destsystemq)) {
        $dessysvar= $row['ReqSystem'];
        $destsystemcount="SELECT `itype` FROM `SENYLRC-SEAL2-STATS` WHERE `DestSystem`='$libsystem' and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  ";
        ...
        #Find which item types were requests
        $destitype=" SELECT distinct (`itype` )  FROM `SENYLRC-SEAL2-STATS`  WHERE `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00' ";
        $destitypeq = mysqli_query($db, $destitype);
        #loop through the results of items from that destination
        while ($row2 = mysqli_fetch_assoc($destitypeq)) {
          $dessysitype= $row2['itype'];
          #Remove any white space
          $destitemcount=" SELECT `fill`  FROM `SENYLRC-SEAL2-STATS` WHERE `Itype`='$dessysitype' and `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
          $destitemcountq = mysqli_query($db,  $destitemcount);
          ...
          #Find what the fill rate is
          $destitemcountfill=" SELECT `fill`  FROM `SENYLRC-SEAL2-STATS` WHERE Fill='1' and `Itype`='$dessysitype' and `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
          $destitemcountfillq = mysqli_query($db,  $destitemcountfill);
          #Count the number of fills
          $destnumfilled_rows = mysqli_num_rows($destitemcountfillq);
          #Get percentage
          ...
          echo " &nbsp&nbsp&nbsp&nbsp&nbsp      $destnumfilled_rows (".$percent_friendly_1.") were filled<br>";
          #Find what the unfill rate is
          $destitemcountunfill=" SELECT `fill`  FROM `SENYLRC-SEAL2-STATS` WHERE Fill='0' and `Itype`='$dessysitype' and `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
          $destitemcountunfillq = mysqli_query($db,  $destitemcountunfill);
          #Count the number of unfilled
          $destnumunfilled_rows = mysqli_num_rows($destitemcountunfillq);
          #Get percentage
          ...
          echo "&nbsp&nbsp&nbsp&nbsp&nbsp   $destnumunfilled_rows (". $percent_friendly_2.") were not filled<br>";
          #Find what the expire rate is
          $destitemcountexfill=" SELECT `fill`  FROM `SENYLRC-SEAL2-STATS` WHERE Fill='4' and `Itype`='$dessysitype' and `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
          $destitemcountexfillq = mysqli_query($db,  $destitemcountexfill);
          #Count the number of expired requests
          $destnumexfilled_rows = mysqli_num_rows($destitemcountexfillq);
          #Get percentage
          ...
          echo "&nbsp&nbsp&nbsp&nbsp&nbsp   $destnumexfilled_rows (". $percent_friendly_3.") were expired<br>";
          #Find what the cancel rate is
          $destitemcountcanfill=" SELECT `fill`  FROM `SENYLRC-SEAL2-STATS` WHERE Fill='6' and `Itype`='$dessysitype' and `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
          $destitemcountcanfillq = mysqli_query($db,  $destitemcountcanfill);
          #Count the number of canceled requests
          $destnumcanfilled_rows = mysqli_num_rows($destitemcountcanfillq);
          #Get percentage
          ...
          echo "&nbsp&nbsp&nbsp&nbsp&nbsp   $destnumcanfilled_rows  (". $percent_friendly_4.") were canceled<br>";
          #Find the numbered not answer yet
          $destitemcountnoanswfill=" SELECT `fill`  FROM `SENYLRC-SEAL2-STATS` WHERE Fill='3' and `Itype`='$dessysitype' and `DestSystem`='$libsystem'  and `ReqSystem`='$dessysvar' and `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'";
          $destitemcountnoanswfillq = mysqli_query($db,  $destitemcountnoanswfill);
          #Count the number of requests not answered yet
          $destnumnoanswfilled_rows = mysqli_num_rows($destitemcountnoanswfillq);
          #Get percentage
...
#Generate the top 10 filling requests
  ...
  while ($row2 = mysqli_fetch_assoc($retval )) {
    ...
    $row_cnt = mysqli_num_rows($retval);
     ...
     $GETREQUESTCOUNTSQLL= "SELECT * FROM `SENYLRC-SEAL2-STATS` WHERE `Timestamp` >= '$startdate 00:00:00' and `Timestamp` <= '$enddate 00:00:00'  and  Fill =4 ORDER BY Destination";
     $retval = mysqli_query($db, $GETREQUESTCOUNTSQLL);
     $row_cnt = mysqli_num_rows($retval);
